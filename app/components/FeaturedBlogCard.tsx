 ...
 * layout inside the same .card grain-paper container as BlogCard. Sits
 * above the category filter + card grid.
 */
export function FeaturedBlogCard({ post }: FeaturedBlogCardProps) {
  return (
    <a
      href={`/blog/${post.slug}`}
      className="card grain-paper bg-[#FAF9F2] text-ink group !min-h-0 !p-6 md:!p-10 grid md:grid-cols-2 gap-6 md:gap-12 items-stretch"
    >
      {/* Image column — left */}
      <div
